 <!-- Footer -->
 <footer class="bg-dark-brown text-white py-12 px-4">
    <div class="container mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 text-center md:text-left">
            <!-- Brand -->
            <div class="animate-on-scroll">
                <h3 class="text-2xl font-bold mb-4">Joyful</h3>
                <p class="text-gray-200 mb-4">Bringing joy to your everyday life through carefully curated products.</p>
                <div class="flex space-x-4 justify-center md:justify-start">
                    <a href="https://www.facebook.com/Joyfulegy" target="_blank" class="text-gray-200 hover:text-red-600 transition-colors">
                        <i class="fab fa-facebook text-xl"></i>
                    </a>
                    <a href="https://www.instagram.com/Joyfulegy/" target="_blank" class="text-gray-200 hover:text-red-600 transition-colors">
                        <i class="fab fa-instagram text-xl"></i>
                    </a>

                    <a href="mailto:Joyful@joyfulegy.com" target="_blank" class="text-gray-200 hover:text-red-600 transition-colors">
                      <i class="fa-regular fa-envelope text-xl"></i>
                    </a>

                </div>

            </div>

            <!-- Shop -->
            <div class="animate-on-scroll">
                <h4 class="font-bold mb-4 uppercase">Candles</h4>
                <ul class="space-y-2">
                    @foreach ($products->take(5) as $product)
                    <li><a href="{{ route('product.show', $product->slug) }}" class="text-gray-200 hover:text-red-600 transition-colors capitalize">{{ $product->name }}</a></li>
                    @endforeach

                </ul>
            </div>

            <!-- Support -->
            <div class="animate-on-scroll">
                <h4 class="font-bold mb-4">SUPPORT</h4>
                <ul class="space-y-2">

                    <li><a href="{{ route('contact.index') }}" class="text-gray-200 hover:text-red-600 transition-colors capitalize">Contact Us</a></li>
                    <li><a href="{{ route('terms') }}" class="text-gray-200 hover:text-red-600 transition-colors capitalize">Terms & Conditions</a></li>
                    <li><a href="{{ route('return') }}" class="text-gray-200 hover:text-red-600 transition-colors capitalize">Shipping & Returns</a></li>
                    <li><a href="{{ route('privacy') }}" class="text-gray-200 hover:text-red-600 transition-colors capitalize">privacy Policy</a></li>

                </ul>
            </div>



        </div>
        <div class="border-t border-gray-600 pt-8 text-center text-gray-200 animate-on-scroll">
            <p>&copy; 2025 Joyful. All rights reserved.</p>
            <a href="https://medsite.dev" class=" py-0 pointer-none hover:pointer-none" style="color: transparent; cursor: default; pointer-events: none; text-decoration: none;">>Developed by Medsite</a>
        </div>
    </div>
</footer>


