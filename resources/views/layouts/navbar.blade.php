...
<div class="container px-8 mx-auto">
  <div class="flex relative justify-between items-center">
      ...
      <div class="flex flex-shrink-0 items-center space-x-3 md:hidden">
          ...
          <!-- User/Login Icon -->
          <a href="{{ route('login') }}" class="relative transition-colors hover:text-red-600"
      ...
      <div class="pt-2 border-t border-gray-200">
        <button @click="mobileProductsOpen = !mobileProductsOpen"
          ...
          <svg class="w-4 h-4 transition-transform duration-200"
               :class="mobileProductsOpen ? 'rotate-45' : ''"
        ...
        <div x-show="mobileProductsOpen"
          ...
          <div class="pt-2 pl-4 space-y-1">
            <a href="{{ route('products.index') }}" class="block py-1 text-sm text-gray-700 transition-colors hover:text-red-600" @click="mobileMenuOpen = false">All Products</a>
            @if($products && $products->isNotEmpty())
              @foreach($products->take(10) as $product)
              <a href="{{ route('product.show', $product->slug) }}" class="block py-1 text-sm text-gray-700 transition-colors hover:text-red-600" @click="mobileMenuOpen = false">{{ $product->name }}</a>
              @endforeach
            @else
              <p class="block py-1 text-sm text-gray-500 italic">No products available</p>
            @endif
          </div>
        </div>
      </div>
