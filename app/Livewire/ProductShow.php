...
class ProductShow extends Component
{
    ...
    public $selectedWickType = null;
    public $availableWickTypes = [];
    ...
    public function mount(Product $product)
    {
        ...
        if ($this->product->variants->isNotEmpty()) {
            ...
            $this->selectedWickType = $this->selectedVariant->wick_type ?? null;
            // Prepare available wick types (for the selected size if present)
            $this->prepareAvailableWickTypes();
    ...
    protected function prepareAvailableWickTypes()
    {
        if (!$this->product || $this->product->variants->isEmpty() || !$this->selectedSize) {
            $this->availableWickTypes = [];
            return;
        }
        $wicks = $this->product->variants->where('size', $this->selectedSize)
            ->pluck('wick_type')->filter()->unique()->values()->toArray();
        $this->availableWickTypes = $wicks;
    ...
    protected function convertProductPrices()
    {
        ...
        try {
            ...
            // Convert variant prices (products don't have prices, only variants do)
    ...
    protected function convertVariantPrice($variant)
    {
        ...
        try {
            ...
            if ($variant && $variant->compare_price && $variant->compare_price > 0) {
    ...
    public function selectVariant($variantId)
    {
        ...
        if ($this->selectedVariant) {
            ...
            // Update size/wick selections to match the selected variant
            $this->selectedSize = $this->selectedVariant->size ?? null;
            $this->selectedWickType = $this->selectedVariant->wick_type ?? null;
            $this->prepareAvailableWickTypes();
    ...
    /**
     * User selected a size. Show available wick types for that size
     * and pick a sensible default variant (prefer in-stock).
     */
    public function selectSize($size)
    {
        ...
        // Find variant for selected size and wick_type
        $variant = $this->product->variants->where('size', $size)
            ->where('wick_type', $this->selectedWickType)->first();
        if ($variant) {
            $this->selectedVariantId = $variant->id;
            $this->selectedVariant = $variant;
            $this->convertVariantPrice($variant);
            $this->quantity = min(max(1, $this->quantity), max(1, $variant->stock));
        }
    }

    /**
     * User selected a wick type for the current size.
     */
    public function selectWick($wick)
    {
        $this->selectedWickType = $wick;
        $this->prepareAvailableWickTypes();
        $variant = $this->product->variants->where('size', $this->selectedSize)
            ->where('wick_type', $this->selectedWickType)->first();
        if ($variant) {
            $this->selectedVariantId = $variant->id;
            $this->selectedVariant = $variant;
    ...
    public function render()
    {
        $relatedProducts = Product::where('category_id', $this->product->category_id)
            ...
            ->where('active', true)
            ->with(['media', 'variants:id,product_id,price,compare_price,stock'])
            ->take(4)
            ->get();

        // Convert related products variant prices to current currency
    ...
    protected function convertRelatedProductsPrices($relatedProducts)
    {
        ...
        try {
            ...
            // Convert all related product variant prices
