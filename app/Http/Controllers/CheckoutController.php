...
class CheckoutController extends Controller
{
    ...
    public function processAuthenticatedCheckout(Request $request)
    {
        ...
        try {
            ...
            // If payment is successful (Paymob/PayPal), update payment_status to 'paid'
            if (in_array($validated['payment_method'], ['paymob', 'paypal']) && (!isset($result['requires_frontend_processing']) || !$result['requires_frontend_processing'])) {
    ...
    protected function createOrder(array $data, Customer $customer, ?User $user, bool $isGuest)
    {
        ...
        if (isset($data['payment_method']) && in_array($data['payment_method'], ['paymob', 'paypal'])) {
            $paymentStatus = 'paid';
        }
        ...
        foreach ($cart as $item) {
            ...
            try {
                $order->items()->create([
                    ...
                    'variant_id' => $item['attributes']['variant_id'] ?? null,
                    'variant_size' => $item['attributes']['size'] ?? $item['attributes']['variant_size'] ?? null,
                    'variant_wick_type' => $item['attributes']['wick_type'] ?? $item['attributes']['variant_wick_type'] ?? null,
